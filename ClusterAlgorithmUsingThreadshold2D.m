...
T2_SN(:,:,:) = 0;
img1_2d = (SWI(:,:,slicess));
img2_2d = (SWIphase(:,:,slicess));
img3_2d = (T1_SN(:,:,slicess));
img4_2d = (T2_SN(:,:,slicess));
img5_2d = (Fl_SN(:,:,slicess));
...
h = waitbar(0,['Loading']);
totalOp = numberLines*numberColumns;
...
listOfFamilies{1}{1} = [img_resul(firstI,firstJ).c1,img_resul(firstI,firstJ).c2,img_resul(firstI,firstJ).c3,img_resul(firstI,firstJ).c4,img_resul(firstI,firstJ).c5]';

img_resul(firstI,firstJ).family = 1;
...
%         if(i==firstI) && (j==firstJ) %changed 1 for firstI/J
%         img_resul(i,j).family = (contador);
% %         listOfFamilies{i}{i} = [img_resul(i,j).c1,img_resul(i,j).c2,img_resul(i,j).c3,img_resul(i,j).c4,img_resul(i,j).c5]';
%         else
...
%             listOfFamiliesPositions{numberFamilies+1}{1} = [i,j];
            img_resul(i,j).family =  numberFamilies + 1;
          end       
...
for i=1:numberLines
    for j=1:numberColumns
        matrizFamilia(i,j,slicess) = img_resul(i,j).family;
    end
end
